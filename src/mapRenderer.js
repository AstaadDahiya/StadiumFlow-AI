/**
 * @fileoverview SVG stadium map renderer with crowd density heatmap visualization.
 * Renders an interactive SVG stadium map with zone click handling and restroom overlay.
 * @module mapRenderer
 */
...
/** @type {boolean} Whether the restroom overlay is currently visible */
let mapRestroomsActive = false;

/** @type {Array<{x: number, y: number, color: string}>} Current restroom markers data */
let currentRestroomsData = [];

/**
 * Renders the stadium SVG heatmap and stat cards into the DOM.
 * Binds click handlers to each stadium zone for detail viewing.
 *
 * @returns {void}
 */
export function renderMap() {
  try {
    ...
    const statsEl = document.getElementById(DOM_IDS.MAP_STATS);
    if (statsEl) {
    ...
    console.error('[MapRenderer] Failed to render map:', error);
  }
}

/**
 * Constructs the stadium SVG string with heatmap-colored zones.
 *
 * @returns {string} Raw SVG markup string.
 * @private
 */
function buildStadiumSVG() {
  ...
  <svg viewBox="0 0 400 320" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Stadium crowd density map">
    ...
    <g font-size="8" font-weight="600" fill="var(--bg-primary)" text-anchor="middle">
      <text x="200" y="32">N-UP</text><text x="200" y="72">N-LO</text>
      <text x="200" y="255">S-LO</text><text x="200" y="285">S-UP</text>
      <text x="36" y="163">W-UP</text><text x="68" y="163">W-LO</text>
      <text x="332" y="163">E-LO</text><text x="363" y="163">E-UP</text>
    </g>
...
/**
 * Displays the zone detail modal with occupancy data for a specific zone.
 *
 * @param {object} zone - The zone data object from ZONES.
 * @param {string} zone.name - Display name of the zone.
 * @param {string} zone.section - Section identifier string.
 * @param {number} zone.density - Occupancy ratio (0–1).
 * @param {number} zone.current - Current number of occupants.
 * @param {number} zone.capacity - Maximum capacity.
 * @returns {void}
 */
export function showZoneDetail(zone) {
  if (!zone || typeof zone.density !== 'number') {
    console.warn('[MapRenderer] Invalid zone data provided to showZoneDetail');
    return;
  }

  const modal = document.getElementById(DOM_IDS.ZONE_DETAIL_MODAL);
  const title = document.getElementById(DOM_IDS.ZONE_DETAIL_TITLE);
  const body = document.getElementById(DOM_IDS.ZONE_DETAIL_BODY);
...
/**
 * Hides the zone detail modal.
 *
 * @returns {void}
 */
export function hideZoneDetail() {
  document.getElementById(DOM_IDS.ZONE_DETAIL_MODAL)?.classList.remove('active');
}

/**
 * Toggles the restroom overlay on the stadium SVG map.
 * Generates random occupancy levels for each restroom marker.
 *
 * @returns {void}
 */
export function toggleMapRestrooms() {
  mapRestroomsActive = !mapRestroomsActive;
  const btn = document.getElementById(DOM_IDS.FIND_RR_BTN);
  if (!btn) { return; }
